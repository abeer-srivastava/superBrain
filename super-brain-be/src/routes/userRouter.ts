...
router.post("/signup", async (req,res)=>{
    ...
    if(!data)return res.status(411).json({message:"Validation Failed"});
    ...
    try { 
        ...
        if(existingUser)return res.status(411).json({message:"User already exists"})
...
try {
        ...
        if(!data)return res.status(411).json({message:"Validation failed for signin"});
        const {email,password}=data;
        if(!email || !password) return res.json("Valid Field are Required ");
        // db call
        ...
        if(existingUser){
            ...
            if(!comparePassword) return res.json({message:"Password doesnot match"});
